...
import { resolveAgentSpecDefinition } from "../agent-definition.tsx";
...
export interface EmitOptions {
  ...
  /** Emit a `getTools()` block (agents 0.17 `agentTool`) for tool-slot bindings
   *  whose provider is the ROOT. OFF by default — the block targets the 0.17 API
   *  and is not executed on the proven 0.8.x reconcile runtime (version-gated per
   *  docs/agent-tools-investigation.md), so schemaless/slotless fixtures and the
   *  live demo are byte-identical. */
  agentTools?: boolean;
  /** Tool-slot bindings discovered from the composition (src/compile/slots.ts);
   *  only bindings whose `provider` is the root become its getTools entries. */
...
/** A JSDoc block embedding an agent's human-facing contract in the generated
 *  module: `description`, `displayName`, and — since a zod schema does not
 *  serialize to source text — the spec schemas the boundary validates against BY
...
function agentDoc(spec: AnyAgentSpec, componentName: string): string {
  ...
  if (spec.inputSchema) contracts.push(`input=${componentName}.spec.inputSchema`);
  if (spec.outputSchema) contracts.push(`output=${componentName}.spec.outputSchema`);
  if (contracts.length) lines.push(` * @boundarySchema ${contracts.join(" ")} (validated at the boundary)`);
  return lines.length ? `/**\n${lines.join("\n")}\n */\n` : "";
}
...
export function emitCloudflare(
  ...
  const rootDefinition = resolveAgentSpecDefinition(
    spec,
    spec.sampleProps ?? {},
  );
  ...
  const kids = children.map((c) => {
    const definition = resolveAgentSpecDefinition(
      c.spec,
      c.sampleProps ?? c.spec.sampleProps ?? {},
    );
  ...
  // agentTools mode (version-gated): tool-slot bindings whose PROVIDER is the
  // root become its getTools() entries — one model-tool per prop key, dispatching
  // the bound child (referenced by class + spec, exactly the 0.17 agentTool shape).
  const childByKind = new Map(kids.map((k) => [k.spec.agentName, k]));
  const rootToolEntries = (opts.agentTools ? (opts.toolSlots ?? []) : [])
    .filter((b) => b.provider === spec.agentName)
    .map((b) => {
      const child = childByKind.get(b.childKind);
      if (!child) throw new Error(`agentTools: no child registered for tool-slot kind "${b.childKind}"`);
      return { toolName: b.toolName, className: child.className, exportName: child.exportName };
    });
  const getToolsBlock = rootToolEntries.length
    ? `

  /** GENERATED (agents 0.17 \`agentTool\`, version-gated). Each tool-slot binding
   *  at this agent's composition site is a model-tool named after the prop key,
   *  dispatching the child agent — schema'd by the child's own spec. This targets
   *  the documented 0.17 API (the child runs as a per-tool-call ctx.exports facet;
   *  inputSchema is validated at the model boundary) and requires \`agents@>=0.17.1\`
   *  plus a Think/AIChatAgent child; it is NOT executed on the 0.8.x reconcile
   *  runtime this module otherwise targets. See docs/agent-tools-investigation.md. */
  getTools() {
    return {
${rootToolEntries
        .map(
...
`
  ...
  const inertDefinitionDiagnostics = [
    { name: spec.agentName, spec, definition: rootDefinition },
    ...kids.map((kid) => ({
      name: kid.spec.agentName,
      spec: kid.spec,
      definition: kid.definition,
    })),
  ]
    .filter((candidate) => Boolean(candidate.spec.resolveDefinition))
    .map(
      ({ name }) =>
        `// [cloudflare-reconcile-definition-inert] ${name}: render() prompt remains available through promptFor(); model execution, AI SDK tools, skills, and MCP clients require the Cloudflare model-driven target.`,
    )
    .join("\n");
...
${inertDefinitionDiagnostics ? `${inertDefinitionDiagnostics}\n` : ""}/*
...
${agentDoc(k.spec, k.exportName)}export class ${k.className} extends FiberAgentBase<ChildRuntimeState & Record<string, unknown>> {
