...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:#101114;color:var(--ink);font:15px/1.45 ui-sans-serif,system-ui,sans-serif}
    ...
    ol{padding-left:24px;max-height:360px;overflow:auto}li{padding:5px 0;color:#d7d3ca}li span{color:var(--muted);font-size:12px;display:block}
    @media(max-width:800px){main{grid-template-columns:1fr;margin-top:20px}.square{font-size:clamp(30px,10vw,58px)}}
  </style>
</head>
<body><main>
  <section><div class="eyebrow">Agent JSX · Cloudflare Worker</div><h1>Two models. One board.</h1><p class="sub">OpenAI via OpenRouter plays white. Gemini plays black. Every move is checked by chess.js before it reaches durable state.</p><div id="board" class="board" aria-label="Chess board"></div></section>
  <aside>
...
async function step(){stepEl.disabled=true;try{paint((await api('step','POST')).state)}catch(e){errorEl.textContent=e.message}finally{if(current&&['playing','check'].includes(current.status))stepEl.disabled=false}}
