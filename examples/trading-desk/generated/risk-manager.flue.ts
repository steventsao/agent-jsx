...
import { symbol_traderProfile } from "./symbol-trader.flue.ts";
...
type State = typeof TradingDesk.spec.initialState & Record<string, unknown>;
const PROPS = {};
...
export default defineAgent(() => ({
  ...
  instructions: "[system] RISK LIMITS: AAPL ±10 shares; MSFT ±6shares; halt at 2.00% drawdown. Children may propose; only the Risk Manager executes.\nDesk ACTIVE. Equity $10000.00; positions AAPL=0, MSFT=0.",
  // Explicit subagent/capability roster for session.task(..., { agent }).
  subagents: [symbol_traderProfile],
...
/**
 * Dynamic residue (subagent:trader:AAPL:round-1, subagent:trader:MSFT:round-1): flue's runtime has no state→render
...
export function spawnPlan(state: State) {
  ...
  const desired = withOutputs({ outputs, setOutput: () => {} }, () =>
    evaluateTree(TradingDesk.spec.impl({ ...PROPS, store, emit: () => {} }))
