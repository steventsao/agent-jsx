...
`turn` data and `onTurn={handleTurn}` capability. `Agent` is a generic adapter over an
`agentComponent`, so the existing compiler sees normal subagent records:
...
Generate the readable Flue binding table, player profiles, and reactive
workflow:
