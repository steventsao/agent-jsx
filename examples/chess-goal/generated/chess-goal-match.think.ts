...
import { OpenAISeat } from "../players.tsx";
import { GeminiSeat } from "../players.tsx";
...
export class ChessGoalMatchDurable extends ThinkAgentBase<ChessGoalMatchState> {
  ...
  override getTools(): ToolSet {
    return {
      openai_chess_player: agentTool(OpenaiChessPlayerDurable, { description: OpenAISeat.spec.description ?? "openai_chess_player", displayName: OpenAISeat.spec.displayName, inputSchema: modelToolInputSchema(OpenAISeat.spec.inputSchema), outputSchema: OpenAISeat.spec.outputSchema }),
      gemini_chess_player: agentTool(GeminiChessPlayerDurable, { description: GeminiSeat.spec.description ?? "gemini_chess_player", displayName: GeminiSeat.spec.displayName, inputSchema: modelToolInputSchema(GeminiSeat.spec.inputSchema), outputSchema: GeminiSeat.spec.outputSchema }),
    };
  }
}
...
type OpenaiChessPlayerState = typeof OpenAISeat.spec.initialState & Record<string, unknown>;
const OPENAI_CHESS_PLAYER_PROPS = {"turn":{"ply":0,"side":"white","fen":"rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1","legalMoves":["a2a3","a2a4","b2b3","b2b4","c2c3","c2c4","d2d3","d2d4","e2e3","e2e4","f2f3","f2f4","g2g3","g2g4","h2h3","h2h4","b1a3","b1c3","g1f3","g1h3"],"history":[],"lastError":null}} as const;


export class OpenaiChessPlayerDurable extends ThinkAgentBase<OpenaiChessPlayerState> {
  initialState = { ...OpenAISeat.spec.initialState } as OpenaiChessPlayerState;
  override getModel() { return OpenAISeat.spec.model ?? "openrouter/openai/gpt-5-mini"; }
  ...
  protected renderTree(): unknown {
    return OpenAISeat.spec.impl({ ...this.turnProps(OPENAI_CHESS_PLAYER_PROPS), store: this.boundStore<OpenaiChessPlayerState>(), emit: () => {} } as never);
  }
  protected imperativePrompt(state: OpenaiChessPlayerState): string {
    return OpenAISeat.spec.getPrompt?.(state) ?? "";
  }
...
type GeminiChessPlayerState = typeof GeminiSeat.spec.initialState & Record<string, unknown>;
...
export class GeminiChessPlayerDurable extends ThinkAgentBase<GeminiChessPlayerState> {
  initialState = { ...GeminiSeat.spec.initialState } as GeminiChessPlayerState;
  override getModel() { return GeminiSeat.spec.model ?? "openrouter/google/gemini-2.5-flash"; }
  ...
  protected renderTree(): unknown {
    return GeminiSeat.spec.impl({ ...this.turnProps(GEMINI_CHESS_PLAYER_PROPS), store: this.boundStore<GeminiChessPlayerState>(), emit: () => {} } as never);
  }
  protected imperativePrompt(state: GeminiChessPlayerState): string {
    return GeminiSeat.spec.getPrompt?.(state) ?? "";
  }
