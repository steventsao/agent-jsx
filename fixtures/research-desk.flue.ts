...
import { tool_workerProfile } from "./tool-worker.flue.ts";
import { tool_summarizerProfile } from "./tool-summarizer.flue.ts";
...
type State = typeof ResearchDesk.spec.initialState & Record<string, unknown>;
const PROPS = {};
...
export default defineAgent(() => ({
  ...
  instructions: "[system] Research desk — delegate a query to both a researcher and a summarizer.\nidle",
  // Explicit subagent/capability roster for session.task(..., { agent }).
  subagents: [tool_workerProfile, tool_summarizerProfile],
...
/**
 * Dynamic residue (subagent:research, subagent:summary): flue's runtime has no state→render
...
export function spawnPlan(state: State) {
  ...
  const desired = withOutputs({ outputs, setOutput: () => {} }, () =>
    evaluateTree(ResearchDesk.spec.impl({ ...PROPS, store, emit: () => {} }))
